...
cond3 <- args[3] #3rd variable
cond4 <- args[4] #4th variable


#Import the processed event align files
condition1<- read.csv(cond1, sep="\t")
condition2<- read.csv(cond2, sep="\t")
condition3<- read.csv(cond3, sep="\t")
condition4<- read.csv(cond4, sep="\t")
...
condition3$position<- condition3$position+3
condition4$position<- condition4$position+3
...
condition3$Pos<- paste(condition3$contig, condition3$position, sep="_")
condition4$Pos<- paste(condition4$contig, condition4$position, sep="_")
...
condition3$sample<- rep("condition3", nrow(condition3))
condition4$sample<- rep("condition4", nrow(condition4))

#Merge all the tables
all<- rbind(condition1,condition2,condition3,condition4)
