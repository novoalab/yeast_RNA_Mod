...
def df_proc (small_files_dir, outprefix, strands=1):
	...
	if df_is_not_empty (df_plus):
		...
		print ("no results for plus strand variants", file=sys.stderr)
	if strands == 2:
		...
		if df_is_not_empty (df_minus):
			...
			print ("no results for minus strand variants", file=sys.stderr)
	return outs 
...
def df_groupy(df, out):
	...
	for i,j in gb.iterrows():
		...
		acgt = ''
		try:
			bases = np.array(  [ ele for ele in  j['bases'].str.split(':').values[0][:-1] if ele] ).astype(int)		
			...
			outfh.write ("{},{},{},{},{},{},{},{},{}\n".format(i,cov,qmn,qme,qst,mis,ins,_de,":".join(map (str,acgt))))
		except:
...
if args.type.lower() == 't':
	var_files = df_proc (tmp_dir, prefix, 1)
